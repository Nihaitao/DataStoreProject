...
namespace DataStoreProject.Business
{
    public class DisciplineMapper : BaseMapper
    {
        ...
        private bool InTheList(List<W_CourseModel> CourseList, int DisciplineId)
        {
            foreach (W_CourseModel Course in CourseList)
            {
                if (("," + Course.DisciplinePIds + ",").IndexOf("," + DisciplineId + ",") >= 0)
                    return true;
            }
            return false;
        }
        ...
        public List<DisciplineModel> GetAllDisciplineForStudent(int System_Station_ID)
        {
            string str = SqlMapper.QueryForObject<string>("GetAllDisciplineForStudentString", null);
            List<DisciplineModel> List = SqlMapper.QueryForList<DisciplineModel>("GetAllDisciplineForStudent", new { System_Station_ID, str }).ToList();
            //添加子集
            List<DisciplineModel> NodeList = new List<DisciplineModel>();
            List<DisciplineModel> List1 = new List<DisciplineModel>();
            if (List != null && List.Count > 0)
            {
                NodeList = List.FindAll(X => X.CID == 0);
                if (NodeList != null && NodeList.Count > 0)
