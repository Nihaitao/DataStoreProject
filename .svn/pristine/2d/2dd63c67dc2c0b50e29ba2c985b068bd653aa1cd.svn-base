...
namespace DataStoreProject.Web.Controller
{
    public class OrderController : StationBaseApiController
    {
        ...
        {
            return Success(mapper.AddOrderStudent(order,this.System_Station_ID,this.SafeGetStuId));
        }
        ...
        /// <returns></returns>
        [HttpPost]
        public dynamic PutInShoppingCar(OrderModel OrderModel)
        {
            return Success(mapper.PutInShoppingCar(OrderModel.Course_IDs, this.SafeGetStuId));
        }
        ...
        //{
        //    return Success(mapper.GetOrderDetailListByStuId(model, this.SafeGetStuId, this.System_Station_ID), model.PageIndex == 1 ? mapper.GetOrderDetailListByStuIdCount(model, this.SafeGetStuId, this.System_Station_ID) : 0);
        //}
        ...
        {
            return Success(mapper.GetShoppingCar(this.SafeGetStuId));
        }
        ...
        {
            return Success(mapper.MoveOutShoppingCar(model,this.SafeGetStuId));
        }
        ...
        /// <returns></returns>
        [HttpPost]
        public dynamic AddOrderFromShoppingCar(PageModel model)
        {
            return Success(mapper.AddOrderFromShoppingCar(this.SafeGetStuId, this.System_Station_ID));
        }
        ...
        public dynamic PaySuccessDoSomething(PostModel post)
        {
            ...
            Logger.Debug("[" + DateTime.Now.ToString() + "]post参数： " + JsonStr);
        ...
        public dynamic GetOrderDetailListByStuId([FromUri]CampusModel model,int PayStatus = -1)
        {
            ...
            {
                //if (string.IsNullOrEmpty(this.SafeGetStuId))
                //{
                //    throw new ApiException("StuId 异常，请重新登录！");
                //}
                stuID = this.SafeGetStuId;
            }
            else
            {
                stuID = model.StuId;
        ...
        {
            return Success(mapper.GetOrderListByCourse(model, this.SafeGetStuId, this.System_Station_ID));
        }

        [HttpGet]
        public dynamic GetMyCourseList([FromUri]PageModel page)
        {
            return Success(mapper.GetMyCourseList(page, this.SafeGetStuId, this.System_Station_ID), page.PageIndex == 1 ? mapper.GetMyCourseCounts(this.SafeGetStuId, this.System_Station_ID) : 0);
        }
    }
}